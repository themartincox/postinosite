      ...
      <div className="bg-soft-gray py-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center text-sm text-gray-600">
            ...
            <Link href={`/${parentService}`} className="hover:text-midnight-blue transition-colors capitalize">
              {parentService.replace('-', ' ')}
            </Link>
            <span className="mx-2">/</span>
            <Link href={`/${parentService}/${categorySlug}`} className="hover:text-midnight-blue transition-colors">
              {categoryName}
            </Link>
            <span className="mx-2">/</span>
      ...
      <section className={`${colorClass.bg} text-white py-20`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl">
            <Badge className="bg-white/20 text-white mb-6">{categoryName} Service</Badge>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Button asChild size="lg" className="bg-white text-midnight-blue hover:bg-white/90">
              ...
              <Button asChild variant="outline" size="lg" className="border-white hover:bg-white hover:text-midnight-blue bg-white/5">
                <Link href="/case-studies" className="text-white hover:text-midnight-blue">
                  <span className="font-heading font-semibold">View Results</span>
                  <ArrowRight className="ml-2 h-5 w-5" />
                </Link>
              </Button>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-3 gap-8">
            {resultMetrics.map((metric, index) => (
              <div key={`metric-${index}-${metric.metric.replace(/\s+/g, '-')}`} className="text-center">
                <div className={`text-4xl lg:text-5xl font-heading font-bold ${colorClass.text} mb-2`}>
                  {metric.value}
                </div>
                <div className="text-lg font-heading font-semibold text-midnight-blue mb-2">
                  {metric.metric}
                </div>
                <p className="font-body text-gray-600 text-sm">{metric.description}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <div className="space-y-3">
                {keyBenefits.map((benefit, index) => (
                  <div key={`benefit-${index}-${benefit.slice(0, 20)}`} className="flex items-start space-x-3">
                    <CheckCircle className={`h-5 w-5 ${colorClass.text} mt-1 flex-shrink-0`} />
            ...
            <div className="bg-white p-8 rounded-xl shadow-2xl border border-gray-100">
              ...
              <div className="space-y-4">
                {resultMetrics.map((metric, index) => (
                  <div key={`result-${index}-${metric.metric.replace(/\s+/g, '-')}`} className={`p-4 bg-gradient-to-r from-${heroColor}/5 to-${heroColor}/10 rounded-lg`}>
                    <div className="flex justify-between items-center">
                      <span className="font-heading font-semibold text-gray-800">{metric.metric}</span>
                      <span className={`text-2xl font-heading font-bold ${colorClass.text}`}>{metric.value}</span>
                    </div>
                    <p className="text-sm font-body text-gray-600 mt-1">{metric.description}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-xl font-body text-gray-600 max-w-3xl mx-auto">
              Comprehensive {serviceName.toLowerCase()} solutions designed to deliver maximum impact for your business.
            </p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {serviceFeatures.map((feature, index) => (
              <Card key={`feature-${index}-${feature.title.replace(/\s+/g, '-')}`} className="bg-white shadow-lg hover:shadow-xl transition-shadow duration-300">
                <CardHeader className="text-center">
                  <div className={`w-16 h-16 ${colorClass.bg} rounded-full flex items-center justify-center mx-auto mb-4`}>
                    <feature.icon className="h-8 w-8 text-white" />
                  </div>
                  <CardTitle className="text-lg font-heading text-midnight-blue">{feature.title}</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="font-body text-gray-600 text-sm text-center">{feature.description}</p>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      <section className="py-20 bg-soft-gray">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-xl font-body text-gray-600 max-w-3xl mx-auto">
              A proven step-by-step approach that ensures your {serviceName.toLowerCase()} delivers exceptional results.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-4 gap-8">
            {processSteps.map((step, index) => (
              <div key={`step-${index}-${step.step}`} className="text-center">
                ...
                <h3 className="text-lg font-heading font-semibold text-midnight-blue mb-3">{step.title}</h3>
          ...
          <Card className="max-w-4xl mx-auto shadow-2xl border-0">
            <CardHeader className={`${colorClass.bg} text-white`}>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-2xl font-heading">{caseStudy.company}</CardTitle>
                  <CardDescription className="text-white/90">{caseStudy.industry}</CardDescription>
                </div>
                <Badge className="bg-white/20 text-white">{caseStudy.metric}</Badge>
              </div>
            </CardHeader>
            <CardContent className="p-8">
              <div className="grid md:grid-cols-2 gap-8">
                <div>
                  <h4 className="font-heading font-semibold text-coral-red mb-3">Challenge</h4>
                  <p className="font-body text-gray-600 mb-6">{caseStudy.challenge}</p>
                </div>
                <div>
                  <h4 className="font-heading font-semibold text-forest-green mb-3">Result</h4>
            ...
            <CardContent className="p-8">
              <div className="grid md:grid-cols-2 gap-8 items-center">
                <div>
                  ...
                  <p className="font-body text-gray-600 mb-6">{investmentInfo.description}</p>
                  {investmentInfo.startingFrom && (
                    <div className={`p-4 ${colorClass.bg}/5 rounded-lg mb-6`}>
                      <div className="flex items-center justify-between">
                        <span className="font-heading font-semibold text-gray-800">Starting from:</span>
                        <span className={`text-2xl font-heading font-bold ${colorClass.text}`}>
                          {investmentInfo.startingFrom}
                        </span>
                      </div>
                    </div>
                  ...
                  {investmentInfo.consultationPrice && (
                    <div className="flex items-center space-x-3 mb-6">
                      ...
                      <span className="font-body text-gray-600">
                        Includes {investmentInfo.consultationPrice} strategy consultation
                      </span>
                    </div>
                  )}
                </div>
                <div className="space-y-4">
                  <Button asChild className={`w-full ${colorClass.bg} hover:${colorClass.bg}/90 text-white`}>
                    <Link href="/growth-consultation">
                      Book Strategy Session
                      <ArrowRight className="ml-2 h-4 w-4" />
                    </Link>
                  </Button>
                  <Button asChild variant="outline" className={`w-full ${colorClass.border} ${colorClass.text} hover:${colorClass.bg} hover:text-white`}>
                    <Link href="/contact">
                      Ask Questions
                      <ArrowRight className="ml-2 h-4 w-4" />
                    </Link>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {relatedServices.map((service, index) => (
              <Card key={`related-${index}-${service.slug}`} className="bg-white shadow-lg hover:shadow-xl transition-shadow duration-300">
                <CardHeader>
                  <CardTitle className="text-lg font-heading text-midnight-blue">{service.name}</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="font-body text-gray-600 text-sm mb-4">{service.description}</p>
                  <Button asChild variant="outline" className={`w-full ${colorClass.border} ${colorClass.text} hover:${colorClass.bg} hover:text-white`}>
                    <Link href={`/${parentService}/${categorySlug}/${service.slug}`}>
                      Learn More
                      <ArrowRight className="ml-2 h-4 w-4" />
                    </Link>
                  </Button>
