        ...
        >
          ...
          <CardContent className="space-y-4 relative z-10">
            <div className="space-y-4">
              {[
                { label: "Inbound", desc: "SEO, Content Strategy, Email Marketing, Social Media" },
                { label: "Branding", desc: "Brand Identity, Design, Website Development, Conversion Funnels" },
                { label: "Strategy", desc: "Market Research, Planning, Analytics, CRO" }
              ].map((item, index) => (
                <motion.div
                  key={item.label}
                ...
                >
                  <Badge variant="secondary" className="bg-forest-green/15 text-forest-green font-heading font-semibold flex-shrink-0 mt-0.5">
                    {item.label}
                  </Badge>
                  <p className="font-heading text-gray-700 text-sm font-medium leading-relaxed">{item.desc}</p>
                </motion.div>
              ))}
            </div>
      ...
      <motion.div variants={cardVariants}>
        ...
        >
          ...
          <CardContent className="space-y-4 relative z-10">
            <div className="space-y-4">
              {[
                { label: "Process", desc: "Business Process Automation, Workflow Optimisation" },
                { label: "Intelligence", desc: "Data Analysis, CRM Automation, Customer Insights" },
                { label: "Custom AI", desc: "Chatbots, Lead Routing, Task Management" }
              ].map((item, index) => (
                <motion.div
                  key={item.label}
                  className="flex items-start space-x-3"
                  ...
                  transition={{ delay: 0.8 + index * 0.1 }}
                >
                  <Badge variant="secondary" className="bg-coral-red/15 text-coral-red font-heading font-semibold flex-shrink-0 mt-0.5">
                    {item.label}
                  </Badge>
                  <p className="font-heading text-gray-700 text-sm font-medium leading-relaxed">{item.desc}</p>
                </motion.div>
