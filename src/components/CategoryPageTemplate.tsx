      ...
      <section className={`${colorClass.bg} text-white py-20`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl">
            <Badge className="bg-white/20 text-white mb-6">{heroSubtitle}</Badge>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Button asChild variant="outline" size="lg" className="border-white hover:bg-white hover:text-midnight-blue bg-white/5">
                <Link href={ctaSecondary.href} className="text-white hover:text-midnight-blue">
                  <span className="font-heading font-semibold">{ctaSecondary.text}</span>
                  <ArrowRight className="ml-2 h-5 w-5" />
                </Link>
              </Button>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {services.map((service, index) => (
              <Card key={`service-${index}-${service.slug}`} className={`bg-white shadow-lg hover:shadow-xl transition-all duration-300 border-l-4 ${colorClass.border}`}>
                <CardHeader>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-3">
                    {service.features.map((feature, featureIndex) => (
                      <div key={`feature-${index}-${featureIndex}-${feature.slice(0, 10)}`} className="flex items-start space-x-3">
                        <CheckCircle className={`h-4 w-4 ${colorClass.text} mt-1 flex-shrink-0`} />
                        <p className="font-body text-gray-600 text-sm">{feature}</p>
                      </div>
                    ))}
                  </div>
                  <Separator />
                  <div className={`${colorClass.bg.replace('bg-', 'bg-')}/5 p-4 rounded-lg`}>
                    <div className={`font-heading font-semibold ${colorClass.text} mb-1`}>
                      Typical Results:
                    </div>
                    <p className="text-sm font-body text-gray-600">{service.resultMetric}</p>
                  </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-xl font-body text-gray-600 max-w-3xl mx-auto">
              A proven methodology that ensures every {categoryName.toLowerCase()} initiative delivers maximum impact.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-4 gap-8">
            {processSteps.map((step, index) => (
              <div key={`step-${index}-${step.step}`} className="text-center">
                ...
                <div className={`text-2xl font-heading font-bold ${colorClass.text} mb-2`}>{step.step}</div>
            ...
            <div>
              <div className="space-y-6">
                {categoryBenefits.map((benefit, index) => (
                  <div key={`benefit-${index}-${benefit.slice(0, 20)}`} className="flex items-start space-x-4">
                    <div className={`w-8 h-8 ${colorClass.bg} rounded-full flex items-center justify-center flex-shrink-0`}>
