  ...
  useEffect(() => {
    ...
    const initParticles = () => {
      ...
      for (let i = 0; i < particleCount; i++) {
        const category = Math.random() < 0.3 ? 'growth' : Math.random() < 0.6 ? 'ai' : 'synergy'
        newParticles.push({
          id: `particle-${i}`,
      ...
      for (let i = 0; i < particlesRef.current.length; i++) {
        ...
        for (let j = i + 1; j < particlesRef.current.length; j++) {
          ...
          if (distance < 100) {
            ...
            ctx.beginPath()
            if (particle.category === 'growth' && otherParticle.category === 'ai') {
              ctx.strokeStyle = `rgba(255, 107, 74, ${0.3 * (1 - distance / 100)})`
            } else if (particle.category === 'growth' && otherParticle.category === 'growth') {
              ctx.strokeStyle = `rgba(52, 168, 138, ${0.2 * (1 - distance / 100)})`
            } else if (particle.category === 'ai' && otherParticle.category === 'ai') {
              ctx.strokeStyle = `rgba(30, 58, 138, ${0.2 * (1 - distance / 100)})`
            } else if (particle.category === 'synergy' || otherParticle.category === 'synergy') {
